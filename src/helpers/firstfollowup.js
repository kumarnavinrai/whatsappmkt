...
function checkDatabase(){
    ...
    con.connect(config, function (err){
		...
		con.query('select * from settings', function (errq, recordset) {
			...
			console.log(recordset[0].f1nod);
			dayspassedaftersendconnect = recordset[0].f1nod;
...
function sendLinkToPreLoadBkp(){
    ...
    con.query('select * from links where user ='+ allusers[0]?.id + ' AND f1_sent = 0 AND f1_cycle = 0 AND participate = 0 ORDER BY id ASC', function (errql, recordsetl) {
...
function checkMemForConnectBtn(){
	...
	if(check === "connect btn present"){
		setTimeout(() => {
			//updating in connect btn present
			con.query('UPDATE links SET f1_cycle = 1 WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
				if(errq) console.log(errq);
...
function checkConnectionAccepted(){
	...
	if(parseInt(eleExists) === 0){
		...
		//updating when connection accepted
		con.query('UPDATE links SET connection_accepted = 1 WHERE id ='+ allusers[0]?.id, function (errq, recordset) {
			if(errq) console.log(errq);
			writeToConsole3();
		});
		...
		if(parseInt(dayspassed) > dayswithdrawonnect){
			withdrawConnectionA();
		}else{
			setTimeout(() => {
				clearConsoleSendConnect();
...
function pasteScriptWcB(){
	...
	//updating when connection is withdrawn
	con.query('UPDATE links SET f1_cycle = 1, connection_withdraw_date = "'+datetosave+'" WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
		if(errq) console.log(errq);
...
function writeToConsoleOmw(){
	...
	robot.keyToggle('control','up');
	robot.setKeyboardDelay(1);
	ncp.copy('var number_of_msg = 0,icon_list_btn_check_msg = document.querySelectorAll(".msg-s-message-list__event"); if(icon_list_btn_check_msg?.length>0){ icon_list_btn_check_msg.forEach((item,i) => {number_of_msg = icon_list_btn_check_msg?.length;if(parseInt(number_of_msg) > 1){ copy("Connection replied!") }}); }');
	setTimeout(() => {
		pasteScriptOmw();
	}, 2000);
}
...
function connectionReplyedOrNot(){
	...
	var month = d.getMonth() + 1;
	var datetosave = d.getFullYear() + "-" + month + "-" + d.getDate();
	//f1 sent should be zero if clinet has responded
	//updating client responded	
	con.query('UPDATE links SET f1_cycle = 1, lead_responded = 1, connection_accepted = 1 WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
		if(errq) console.log(errq);
...
function clickInMsgWindow(){
	...
	con.query("select * from msg where typemsg = 'f1'", function (errq, recordset) {
		if(errq) console.log(errq);
		console.log(recordset);
		msgarray = recordset;
...
function clickOnCrossBtn(){
	//f1 sent
	con.query('UPDATE links SET f1_sent = 1, f1_cycle = 1 WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
		if(errq) console.log(errq);
