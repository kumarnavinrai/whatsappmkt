...
function moveToSalesNavigator(){
    ...
    con.query('select * from joblinkdatacollection where used = 0', function (errq, recordset) {
        if(errq) console.log(errq);
        console.log(recordset);
        ...
        if(recordset.length > 0){
            ...
            for (let index = 0; index < recordset.length; index++) {
                sldata.push(recordset[index].link_of_company);
            }
        ...
        ncp.copy('https://www.linkedin.com/sales'+sldata[0]);
        setTimeout(() => {
...
function getLink(){
	...
	console.log(link);
	setTimeout(() => {
        con.query("UPDATE joblinkdatacollection SET link = '"+link+"', used = 1 WHERE joblinkdatacollection.link_of_company = '" + sldata[0] + "'", function (errq, recordset) {
            if(errq) console.log(errq);
            sldata.splice(0,1);
