...
function moveToSalesNavigator(){
	ncp.copy('https://www.linkedin.com/jobs/');
	setTimeout(() => {
...
function typeSearchCriteria(){
	ncp.copy(search_criteria[0].keyword);
	setTimeout(() => {
		pressEnter();
	}, 2000);
...
function typeSearchCity(){
    var geo = search_criteria[0].geography.split(","); 
	ncp.copy(geo[0]);
	setTimeout(() => {
		pressEnterCity();
	}, 2000);
...
function clickOnPast24(){
    var jp = search_criteria[0].date_posted.trim();
    if(jp === 'Any time'){
        x = 138;
	    y = 164;
    }else if(jp === 'Past week'){
        x = 137;
	    y = 212;
    }else if(jp === 'Past 24 hours'){
        x = 142;
	    y = 256;
    }else if(jp === 'Past month'){
        x = 139;
	    y = 298;
    }else{
        x = 142;
...
function clickOnRemote(){
    var jp = search_criteria[0].onsite.trim();
    if(jp === 'On-site'){
        x = 704;
	    y = 166;
    }else if(jp === 'Remote'){
...
function clickOnContract(){
    var jp = search_criteria[0].job_type.trim();
    if(jp === 'Full-time'){
        x = 699;
	    y = 168;
    }else if(jp === 'Part-time'){
        x = 699;
	    y = 214;
    }else if(jp === 'Contract'){
        x = 698;
...
function startUsingScriptSr(){
	robot.keyToggle('control','down');
		...
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = 'var job_type=document.getElementById("hoverable-outlet-job-type-filter-value"),pageLi=job_type.querySelectorAll("fieldset");pageLi?.length>0&&pageLi.forEach((e,t)=>{var l=e.querySelectorAll(".artdeco-button");l?.length>0&&l.forEach((e,t)=>{-1!==e.getAttribute("aria-label").trim().indexOf("show")&&e.click()})});';
...
function pasteScriptSr(){
	...
	robot.keyToggle('shift','up');
	setTimeout(() => {
		slideSideBarTillBottomJn();
	}, 2000);
}

//{ x: , y:  }
function slideSideBarTillBottomJn(){
	x=604;
	y=244;
	robot.moveMouse(x,y);
	...
	robot.dragMouse(x,y);
	setTimeout(() => {
		slideSideBarTillBottom1Jn();
	}, 5000);
}

function slideSideBarTillBottom1Jn(){
	x=604;
	y=344;
	robot.dragMouse(x,y);
	setTimeout(() => {
		slideSideBarTillBottom2Jn();
	}, 5000);
}

function slideSideBarTillBottom2Jn(){
	x=604;
	y=394;
	robot.dragMouse(x,y);
	setTimeout(() => {
		slideSideBarTillBottom3Jn();
	}, 5000);
}

function slideSideBarTillBottom3Jn(){
	x=604;
	y=550;
	robot.dragMouse(x,y);
	setTimeout(() => {
		slideSideBarTillBottom4Jn();
	}, 5000);
}

function slideSideBarTillBottom4Jn(){
	x=604;
	y=674;
	robot.dragMouse(x,y);
	setTimeout(() => {
		slideSideBarTillTopJn();
	}, 10000);
}


function slideSideBarTillTopJn(){
	x=604;
	y=244;
	robot.dragMouse(x,y);
...
function startUsingScriptNor(){
	robot.keyToggle('control','down');
		robot.keyTap('l');
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = 'var pageLi=document.querySelectorAll("header");pageLi?.length>0&&pageLi.forEach((e,l)=>{var r=e.querySelectorAll("small");r?.length>0&&r.forEach((e,l)=>{var r,a;copy(e.textContent.trim().split(" ")[0])})});';
		ncp.copy(scripttochange);
		setTimeout(() => {
			pasteScriptNor();
		}, 2000);
...
function Nor(){
	var nor = ncp.paste();
	totalresultsfound = parseInt(nor);
	setTimeout(() => {
		startUsingScript();
	}, 2000);
}
...
function startUsingScript(){
	console.log(totalresultsfound);
	robot.keyToggle('control','down');
		robot.keyTap('l');
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = 'var resultarray=[],pageLi=document.querySelectorAll(".scaffold-layout__list-container");pageLi?.length>0&&pageLi.forEach((t,e)=>{var r=[],a=[],n=[],l=[],i=[],o=t.querySelectorAll("a");o?.length>0&&o.forEach((t,e)=>{-1!==t.getAttribute("href").indexOf("/jobs/view/")&&""!==t.textContent.trim()&&(r.push(t.textContent.trim()),a.push(t.getAttribute("href"))),-1!==t.getAttribute("href").indexOf("/company/")&&""!==t.textContent.trim()&&(n.push(t.getAttribute("href")),l.push(t.textContent.trim()))});var h=t.querySelectorAll(".job-card-container__metadata-item");h?.length>0&&h.forEach((t,e)=>{i.push(t.textContent.trim())});for(let u=0;u<r.length;u++)resultarray.push(r[u]+"*"+a[u]+"*"+n[u]+"*"+l[u]+"*"+i[u]);copy(resultarray.join())});';
		ncp.copy(scripttochange);
		setTimeout(() => {
...
function startUsingScriptCP(){
	robot.keyToggle('control','down');
		robot.keyTap('l');
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = 'var resultarray=[],pageLi=document.querySelectorAll(".scaffold-layout__detail");pageLi?.length>0&&pageLi.forEach((e,t)=>{var r=e.querySelectorAll(".jobs-unified-top-card__job-insight");r?.length>0&&r.forEach((e,t)=>{var r=e.querySelectorAll("span");r?.length>0&&r.forEach((e,t)=>{if(-1!==e.textContent.trim().indexOf("employees")){var r;copy(e.textContent.trim().split("\xb7").join("$"))}})})});';
		ncp.copy(scripttochange);
		setTimeout(() => {
			pasteScriptCP();
		}, 2000);
...
function insertDataCP(){
	var techc = ncp.paste();
	techc = techc.split("$");
	var noe = techc[0];
	noe = noe.replace("employees","");
	noe = noe.trim();
	techc = techc[1].trim();
	var dataToInsert = pagearray[0].split("*");
	var company_data = dataToInsert[2]?dataToInsert[2].split("/"):["0","0"]; 
	var insertedIds = [];
	console.log(company_data);

	con.query('SELECT company_id FROM joblinkdatacollection', function (errq, recordsetouter) {
		if(errq) console.log(errq);
		
		for (let index = 0; index < recordsetouter.length; index++) {
			insertedIds.push(recordsetouter[index].company_id);	
		}

		if(insertedIds.indexOf(company_data[2]?.trim()) === -1 && dataToInsert[2] !== 'undefined'){	
			con.query("INSERT INTO joblinkdatacollection (job_title, job_link, link_of_company, name_of_company, geography, size, type_of_company, company_id  ) VALUES ('"+dataToInsert[0]+"', '"+dataToInsert[1]+"', '"+dataToInsert[2]+"', '"+dataToInsert[3]+"', '"+dataToInsert[4]+"', '"+noe+"', '"+techc+"', '"+ company_data[2]?.trim() +"');", function (errq, recordset) {
				if(errq) console.log(errq);

				pagearray.splice(0,1);

				if(pagearray.length > 0){
					setTimeout(() => {
						startUsingScriptNextJob();
					}, 2000);
				}else{
					console.log('pagearray finished');
					totalresultsfound = totalresultsfound - (pageindex+1);
					if(totalresultsfound > 0){
						//click on next page
						noofpage++;
						startUsingScriptChangePge();
					}else{
						robot.keyToggle('control','down');
						robot.keyToggle('shift','down');
						robot.keyTap('i');
...
function startUsingScriptChangePge(){
	robot.keyToggle('control','down');
		robot.keyTap('l');
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = 'var pageindex=changethis,pageLi=document.querySelectorAll(".artdeco-pagination__pages");pageLi?.length>0&&pageLi.forEach((e,a)=>{var l=e.querySelectorAll("li");l?.length>0&&l.forEach((e,a)=>{if(pageindex===a){var l=e.querySelectorAll("button");console.log(l[0]),l[0].click()}})});';
        scripttochange = scripttochange.replace('changethis',noofpage);
		ncp.copy(scripttochange);
		pageindex++;
		setTimeout(() => {
			pasteScriptChangePge();
		}, 2000);
}

function pasteScriptChangePge(){
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
	robot.setKeyboardDelay(1);
...
function startUsingScriptNextJob(){
	robot.keyToggle('control','down');
		robot.keyTap('l');
		robot.keyToggle('control','up');
		robot.setKeyboardDelay(1);
        var scripttochange = '    var pageindex = changethis;var pageLi = document.querySelectorAll(".scaffold-layout__list-container");pageLi?.length > 0&&pageLi.forEach((t, e) => { var loc = t.querySelectorAll(".job-card-container");loc?.length > 0&&loc.forEach((l, m) => {if(pageindex === m){l.click();}});});';
        scripttochange = scripttochange.replace('changethis',pageindex);
		ncp.copy(scripttochange);
		pageindex++;
		setTimeout(() => {
			pasteScriptNextJob();
...
function startConsolingDc(){
		...
		ncp.copy('var resultarray = []; var searchResult = document.getElementById("search-results-container");var pageLi = searchResult.querySelectorAll("li");pageLi?.length > 0&&pageLi.forEach((t, e) => {var ahrefs = t.querySelectorAll("a");ahrefs?.length > 0&&ahrefs.forEach((l, m) => {if(l.getAttribute("data-anonymize") === "company-name"){resultarray.push(l?.href);}});});');
		setTimeout(() => {
			pasteScriptDc();
		}, 2000);
}
...
function getCDcData(){
	...
	if(tempData.length > 0){
		for (let index = 0; index < tempData.length; index++) {
			cid = tempData[index].split("?");
			cid = cid[0].split("/");
			cid = cid[cid.length-1];
			dataToInsert.push({cid:cid,link:tempData[index]});
		}	
	...
	if(dataToInsert.length > 0){
		for (let index = 0; index < dataToInsert.length; index++) {
					if(insertedIds.indexOf(parseInt(dataToInsert[index].cid)) === -1){
						console.log("INSERT INTO sldatacollection (company_id, link) VALUES ('"+dataToInsert[index].cid+"', '"+ dataToInsert[index].link +"');");
						con.query("INSERT INTO sldatacollection (company_id, link) VALUES ('"+dataToInsert[index].cid+"', '"+ dataToInsert[index].link +"');", function (errq, recordset) {
							if(errq) console.log(errq);
