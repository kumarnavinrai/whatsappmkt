...
function checkDatabase(){
    ...
    con.connect(config, function (err){
		...
		con.query('select * from settings', function (errq, recordset) {
			...
			console.log(recordset[0].f2nod);
			dayspassedaftersendconnect = recordset[0].f2nod;
...
function sendLinkToPreLoadBkp(){
    ...
    con.query('select * from links where user ='+ allusers[0]?.id + ' AND f2_sent = 0 AND participate = 0 AND connection_accepted = 1 ORDER BY id ASC', function (errql, recordsetl) {
...
function checkMemForConnectBtn(){
	...
	if(check === "connect btn present"){
		setTimeout(() => {
			con.query('UPDATE links SET f2_sent = 1 WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
				if(errq) console.log(errq);
...
function writeToConsoleOmw(){
	...
	robot.keyToggle('control','up');
	robot.setKeyboardDelay(1);
	ncp.copy('var number_of_msg = 0,icon_list_btn_check_msg = document.querySelectorAll(".msg-s-message-list__event"); if(icon_list_btn_check_msg?.length>0){ icon_list_btn_check_msg.forEach((item,i) => {number_of_msg = icon_list_btn_check_msg?.length;if(parseInt(number_of_msg) > 2){ copy("Connection replied!") }}); }');
	setTimeout(() => {
		pasteScriptOmw();
	}, 2000);
}
...
function connectionReplyedOrNot(){
	...
	con.query('UPDATE links SET f2_sent = 1, lead_responded = 1 WHERE links.id = ' + links[0]?.id, function (errq, recordset) {
		if(errq) console.log(errq);
...
function clickInMsgWindow(){
	...
	con.query("select * from msg where typemsg = 'f2'", function (errq, recordset) {
		if(errq) console.log(errq);
		console.log(recordset);
		msgarray = recordset;
