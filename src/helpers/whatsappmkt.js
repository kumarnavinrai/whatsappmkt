...
function checkPageIsLoadedInsideInterval3(){
	const intervalId = setInterval(function() {
		...
		if(pageLoaded === 'pageloaded'){
			...
			console.log("page not loaded");
			//these are special instructions to click on browser as if msg box of whatsapp does not allow ctrl + shift + i to function start
			x = 130;
			y = 122;
			robot.moveMouse(x, y);
			robot.mouseClick();
			//these are special instructions to click on browser as if msg box of whatsapp does not allow ctrl + shift + i to function end
			setTimeout(() => {
				clickAndCleanConsole3();
			}, 1000);
...
function insertNextMsg1(){
	robot.keyToggle('control','down');
	robot.keyTap('enter');
	robot.keyTap('enter');
	robot.keyToggle('control','up');
	ncp.copy('🌟 Discover the Magic of Sandhu Achaar 🌟');
	setTimeout(() => {
		pasteThisMsg1();
	}, 1000);
}

function pasteThisMsg1(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function insertNextMsg2(){
	ncp.copy('🎁 Exclusive Offer Inside! 🎁');
	setTimeout(() => {
		pasteThisMsg2();
	}, 1000);
}

function pasteThisMsg2(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function insertNextMsg3(){
	ncp.copy('🌸 Embrace a world of elegance, style, and savings with Sandhu Achaar. ✨');
	setTimeout(() => {
		pasteThisMsg3();
	}, 1000);
}

function pasteThisMsg3(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function insertNextMsg4(){
	ncp.copy('https://ibb.co/vjFK17X');
	setTimeout(() => {
		pasteThisMsg4();
	}, 1000);
}

function pasteThisMsg4(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function insertNextMsg6(){
	ncp.copy('ਪਿਆਰੇ ਗਾਹਕ, ਤੁਹਾਡੀ ਕਨੈਕਟ ਬ੍ਰੌਡਬੈਂਡ ਦੀਆਂ ਇੰਟਰਨੈਟ/ਬਰਾਡਬੈਂਡ ਅਤੇ ਲੈਂਡਲਾਈਨ ਟੈਲੀਫੋਨ ਸੇਵਾਵਾਂ ਤੁਹਾਡੇ ਅਗਸਤ ਦੇ ਬਿੱਲ ਦਾ ਭੁਗਤਾਨ ਨਾ ਕਰਨ ਕਾਰਨ ਪਹਿਲਾਂ');
	setTimeout(() => {
		pasteThisMsg6();
	}, 1000);
}

function pasteThisMsg6(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function insertNextMsg7(){
	ncp.copy('ਹੀ ਨਿਯਤ ਮਿਤੀ ਪਾਰ ਹੋ ਜਾਣ ਕਾਰਨ ਰੋਕ/ਰੋਕ ਦਿੱਤੀਆਂ ਗਈਆਂ ਹਨ। ਕਿਰਪਾ ਕਰਕੇ ਨਿਰਵਿਘਨ ਸੇਵਾਵਾਂ ਦਾ ਆਨੰਦ ਲੈਣ ਲਈ ਤੁਰੰਤ ਇਸਦਾ ਭੁਗਤਾਨ ਕਰੋ।');
	setTimeout(() => {
		pasteThisMsg7();
	}, 1000);
}

function pasteThisMsg7(){
	robot.setKeyboardDelay(1);
	robot.keyToggle('control','down');
	robot.keyTap('v');
	robot.keyToggle('control','up');
...
function sendMsg(){
	...
	robot.keyTap('enter');
	setTimeout(() => {
		checkNumberAndMoveForward();
	}, 1000);
}

function checkNumberAndMoveForward(){
	if(mobilenumberformkt === '60555292819'){

	}else{
		mobilenumberformkt= '60555292819';
		setTimeout(() => {
		...
		con.query('SELECT * FROM whatappnumberstocheck WHERE checked <> 1 ORDER BY id LIMIT 1;', function (errq, recordset) {
			...
			if(recordset?.length > 0){
				...
				console.log(recordset);
					...
					con.query("UPDATE whatappnumberstocheck SET checked = 1 WHERE phnumber = '" + mobilenumberformkt +"'", function (errq, recordset) {
						if(errq) console.log(errq);
